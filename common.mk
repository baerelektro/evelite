...
deploy: $(DEPLOYED) ## Deploy a set of contracts marked as initial
	-cat $^
install: $(INSTALLED)
config: tonos-cli.conf.json
tonos-cli.conf.json:
	$(TOC) config --url $(URL) --is_json true --balance_in_tons true  --project_id 2e786c9575af406fa784085c88b5e7e3 --access_key 38f728004a4b40e2a8aa30f8fee45346
conf: $(CONFD)
	-cat $^
...
$(BLD)/%.ployed: $(BLD)/%.shift $(BLD)/%.tvc $(BLD)/%.abi.json $(RKEYS) ## Pre-pay and deploy contract to the blockchain
	...
	$(TOC) -c $(ETC)/$*.conf config --url $(URL) --wc 0 --addr $(file <$<) --abi $(word 3,$^) --is_json true --balance_in_tons true --project_id 2e786c9575af406fa784085c88b5e7e3 --access_key 38f728004a4b40e2a8aa30f8fee45346
...
D$1=$(TOC) -c $(ETC)/$1.conf debug run -m
C$1=$(TOC) -c $(ETC)/$1.conf callx -m
